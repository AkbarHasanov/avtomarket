...
token = '7879945955:AAGUaYb57OpYbh5oRPsBACxvbxiZFlK5Zng'
...
def start_message(message):
    ...
    bot.send_message(659163282, text = f'Клиент запустил бот\n\nЮзернейм: @{message.from_user.username}')
...
def check_callback_data(callback):
    ...
    if callback.data == "🇷🇺Русский":
        ...
        bot.send_message(callback.message.chat.id, text = "Подскажите, пожалуйста, с чем связан ваш вопрос?", reply_markup=markup)
    ...
    if callback.data == "Pey Me Эконом":
        bot.send_message(callback.message.chat.id, text = "Данный сервис временно не доступен.")
    ...
    if callback.data == "Pey Me Iqtisodiyot":
        bot.send_message(callback.message.chat.id, text = "Bu xizmat vaqtincha ishlamaydi.")
...
def event_help_1(message):
    bot.send_message(659163282, text = f"{message.text} Имя пользователя: @{message.from_user.username}")#------------------------------------------------------------------------------------------------------------------------------------
...
def event_help_2(message):
    bot.send_message(659163282, text = f"{message.text} Foydalanuvchi nomi: @{message.from_user.username}")#------------------------------------------------------------------------------------------------------------------------------------
...
def event_help_3(message):
    bot.send_message(659163282, text = f"Отзыв: {message.text} имя пользователя: @{message.from_user.username}")#------------------------------------------------------------------------------------------------------------------------------------
...
def photo_message(message):
    ...
    if counter == 6:
        bot.send_message(659163282, text = f'Клиент ознакомился с планом\n\nЮзернейм: @{message.from_user.username}')
        print(user_avto_list)
